...
esp_err_t rescale_to_int8(std::span<uint32_t> data, std::span<int8_t> out, int32_t data_min, int32_t data_max) {
    constexpr int32_t output_range = INT8_MAX - INT8_MIN;  // 255 (was INT8_MIN - INT8_MAX which was -255)
    uint32_t data_range = data_max - data_min;
    uint32_t scale_down = std::max<uint32_t>(data_range / output_range, 1);  // was std::min, which caused divide-by-zero
