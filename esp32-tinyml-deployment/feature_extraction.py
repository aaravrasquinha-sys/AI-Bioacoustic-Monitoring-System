...
class FeatureConstants:
    ...
    # Global min/max of the training-time float log-mel features (see
    # process_window(..., inference_mode=False)), computed across the
    # reference/calibration dataset. These bound the on-device int8 rescale
    # in feature_extraction.cpp's rescale_to_int8(): if left at 0/0 (the
    # previous default), every live feature clamps to a single value
    # regardless of input, which silently breaks all on-device inference.
    mel_range_min: np.int32 = 0
    mel_range_max: np.int32 = 0
...
def make_constants(win_samples, sample_rate,  window_scaling_bits,
                   ...
                   mel_range_min: int = 0, mel_range_max: int = 0):
    ...
    return FeatureConstants(
        ...
        mel_range_min=mel_range_min,
        mel_range_max=mel_range_max,
    )
...
def convert_constants(c: FeatureConstants):
    ...
    feature_config.FeatureConfigAddMelRangeMin(builder, int(c.mel_range_min))
    feature_config.FeatureConfigAddMelRangeMax(builder, int(c.mel_range_max))
